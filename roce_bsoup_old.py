        ...
        for date_row in rows.find_all("div", {"class": "D(ib)"}):
            if date_row.text == "Breakdown":
                ...
                for dates in chart_dates[2:]:
                    reformatted_date = datetime.strptime(
                        dates.text, "%m/%d/%Y"
                    ).strftime("%m%d%Y")
...
for url in urls:
    if "balance-sheet" in url:
        ...
        balance_sheet_dict = {
            k: v
            for k, v in zip(
                balance_sheet_keys, zip(total_assets_values, total_current_liabilities)
            )
        }
        ...
        financials_dict = {
            k: v
            for k, v in zip(
                operating_income_or_loss_keys, operating_income_or_loss_values
            )
        }
...
for key, values in balance_sheet_dict.items():
    if key.endswith("{}".format(year)):
        ...
        print(
            f"Year: {key}, Total assets: {total_asset}, Total current liabilities: {current_liabilities}, Operating Income or Loss: {balance_sheet_keys}"
        )
