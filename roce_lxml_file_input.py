...
tickerSymbols = ["DOW","MMM","HD","CVX","IBM","WBA","MCD","CAT","NKE","BA","CSCO","KO","MSFT","PFE","XOM","VZ","UTX","WMT","AXP","DIS","INTC","V","GS","PG","JNJ","UNH","JPM","TRV","MRK","AAPL"]
...
for i in range (0, len(tickerSymbols)):
  try:
      ...
      for x in range(0, len(total_assets[0])):
        ...
        roce = 100*float(re.sub("[^\d\.]", "", Operating_Income_or_Loss[0][x+1]))/(float((re.sub("[^\d\.]", "", total_assets[0][x])))-float((re.sub("[^\d\.]", "", Total_Current_Liabilities[0][x]))))
